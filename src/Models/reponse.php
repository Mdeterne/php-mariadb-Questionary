...
class Reponse {
    ...
    public function saveFullResponse($surveyId, $answers) {
        ...
        try {
            $responseId = $this->generateUuidV4();
            $reqResponse = $this->bdd->prepare("INSERT INTO responses (id, survey_id, submitted_at) VALUES (:id, :survey_id, NOW())");
            if (!$reqResponse->execute([':id' => $responseId, ':survey_id' => $surveyId])) {
